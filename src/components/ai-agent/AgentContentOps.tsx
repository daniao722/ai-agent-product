...
export default function AgentContentOps() {
  ...
  // 自动承接模式：开启后接收访客分析派单，自动触发营销页生成
  const [autoAccept, setAutoAccept] = useState(true);
  const [autoGeneratedCount, setAutoGeneratedCount] = useState(3);
  const [lastAutoGen, setLastAutoGen] = useState('4 分钟前');

  // 自动承接循环：每 12 秒模拟接收一次派单并自动生成
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50/40 to-teal-50/30 p-6 space-y-6">
      ...
      <header className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <span className={`w-2 h-2 rounded-full ${autoAccept ? 'bg-cyan-500 animate-pulse' : 'bg-gray-400'}`} />
            <Activity className="w-3.5 h-3.5" />
            {autoAccept ? `自动承接中 · 已生成 ${autoGeneratedCount} · ${lastAutoGen}` : '自动承接已关闭'}
          </button>
        </div>
      </header>
